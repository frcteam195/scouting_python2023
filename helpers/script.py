...
query = "SELECT matches.* FROM matches LEFT JOIN matchScoutingTest " \
        "ON (matches.id = matchScoutingTest.eventID) " \
        "AND matches.id = matchScoutingTest.id " \
        "JOIN events ON (events.id = matches.eventID) " \
        "WHERE (((events.currentEvent) = 1) AND ((matchScoutingTest.matchID) is Null))"
#print(query)
...
for col in rsMatches:
    ...
    while i <= 6:
        ...
        query = "SELECT matchID, eventID, matchNum, allianceStationID FROM matchScoutingTest WHERE " + \
        ...
        # if the record does not exist lets grab it again, with team this time, and add it to the matchScouting table
            ...
            values = str(tuple([x[1] for x in items]))
            query = "INSERT INTO matchScoutingTest " + columns + " VALUES " + values
            cursor.execute(query)
            conn.commit()
        i += 1
...
print()
print ("looping through matchScoutingTest records and comparing teams in each record to the matches table")
print ("    fixing team # if different to fix any typos if we had to enter the schedule by hand")
updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.red1 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 1"
cursor.execute(updateQuery)
conn.commit()

updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.red2 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 2"
cursor.execute(updateQuery)
conn.commit()

updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.red3 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 3"
cursor.execute(updateQuery)
conn.commit()

updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.blue1 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 4"
cursor.execute(updateQuery)
conn.commit()

updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.blue2 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 5"
cursor.execute(updateQuery)
conn.commit()

updateQuery = "UPDATE matchScoutingTest INNER JOIN matches ON (matchScoutingTest.matchID = matches.id) " \
              "INNER JOIN events ON (events.id = matches.eventID) " \
              "SET matchScoutingTest.team = matches.blue3 " \
              "WHERE events.currentEvent = 1 AND matchScoutingTest.allianceStationID = 6"
cursor.execute(updateQuery)
conn.commit()


# add team match numbers by looping back through each teams matches and counting
cursor.execute("SELECT DISTINCT matchScoutingTest.team "
               "FROM matchScoutingTest INNER JOIN events ON matchScoutingTest.eventID = events.id "
               "AND ((events.currentEvent) = 1) "
               "ORDER BY matchScoutingTest.team; ")
rsTeams = cursor.fetchall()
# print(rsTeams)

print()
print ("looping through matchScoutingTest records and creating teamMatchNum values")
...
for team in rsTeams:
    # print(team[0])
    cursor.execute("SELECT matchScoutingTest.id FROM matchScoutingTest INNER JOIN events "
                   "ON matchScoutingTest.eventID = events.id AND ((events.currentEvent) = 1) "
                   "WHERE matchScoutingTest.team = "
                   + team[0] + " ORDER BY matchScoutingTest.id; ")
    ...
    for match in rsTeamMatchScouting:
        matchNum += 1
        # print(match[0])
        query = "UPDATE matchScoutingTest SET matchScoutingTest.teamMatchNum = " + str(matchNum) + \
                " WHERE matchScoutingTest.id = " + str(match[0]) + ";"
        # print(query)
        cursor.execute(query)
        conn.commit()
